    ...
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> AddService(AddServiceDto model)
        {
            if (!ModelState.IsValid)
            {
                ...
                var responseMessage = await client.PostAsync("http://localhost:35777/api/Service", stringContent);
        ...
        public async Task<IActionResult> DeleteService(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:35777/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:35777/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto model)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(model);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:35777/api/Service", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
